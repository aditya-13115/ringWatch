...
def create_address(
    ...
    row = {
        ...
        "canonical_address": (
            f"Hard Negative Address {idx}, India"
        ),
        "city": rng.choice(
            [
                "Mumbai",
...
def create_instrument(
    ...
    row = {
        ...
        "bin_or_vpa_hash": make_instrument_hash(
            idx
        ),
        "first_seen_at": pd.Timestamp(
            first_seen
...
def run_hard_negatives():
    ...
    hard_account_ids = accounts[accounts["population_type"] == "hard_negative"]["account_id"].tolist()
    ...
    hostel_accounts = n_total - family_accounts - size_sampler_accounts - office_accounts - discount_accounts - address_change_accounts
    ...
    for _ in range(family_groups):
        ...
        for idx, acc in enumerate(ids):
            ...
            for j in range(3):
                ...
                new_orders.append(make_order(
                    make_order_id(order_counter), acc, shared_device,
                    shared_address, phone, inst, ts,
                    float(rng.integers(1000, 5000)), "fashion",
                    rng.random() < 0.03, "size_fit",
                ))
                order_counter += 1
    ...
    for _ in range(size_sampler_accounts):
        ...
        for j in range(3):
            ...
            new_orders.append(make_order(
                make_order_id(order_counter), acc, device, address,
                phone, inst, created + pd.Timedelta(days=j + 1),
                float(rng.integers(1500, 4000)), "fashion",
                return_flag, "size_fit",
            ))
            order_counter += 1
    ...
    for _ in range(office_groups):
        ...
        for idx, acc in enumerate(ids):
            ...
            for j in range(3):
                new_orders.append(make_order(
                    make_order_id(order_counter), acc, device,
                    shared_address, phone, inst,
                    created + pd.Timedelta(days=j + 1),
                    float(rng.integers(500, 5000)),
                    rng.choice(["grocery", "home", "electronics"]),
                    rng.random() < 0.04, "not_as_described",
                ))
                order_counter += 1
    ...
    for _ in range(discount_accounts):
        ...
        for j in range(8):
            ...
            row = make_order(
                make_order_id(order_counter), acc, device, address,
                phone, inst, ts, float(rng.integers(800, 3500)),
                rng.choice(["fashion", "grocery", "home"]),
                rng.random() < 0.12, "not_as_described",
            )
